...
select * from menu order by m_name;
--2~4 까지 메뉴테이블 정보를 메뉴명으로 가나다 순
select * from (select rownum as rn , m_name , m_price from ( select m_name,m_price from menu order by m_name asc))where rn >=2 and rn<=4;
--길동 이름을 가진 사장님네 메뉴명,가격,식당명,지점명,사장님이름 가격 은 내림차순 가격이 같다면 메뉴명은 가나다순 3~8번째 까지 조회.
select * from (select rownum as rn ,m_name,m_price ,s_name,s_location,c_name from (select m_name,m_price,s_name,s_location,c_name from menu,store,ceo where m_f_no = s_no and s_no = c_no order by m_price desc,m_name asc))where rn between 2 and 5;
--김씨 성을 가진 사장님네 메뉴명,가격,사장님이름,사장님 성별 을 가격이 오름차순으로 정렬을해서 2~3번까지만 


create table store (
s_no number(3)primary key,
s_name varchar2 (30char) not null,
s_location varchar2 (30char)not null,
s_ceo number(3) not null,
s_seat number(5) not null
);
...
--무 삭제
delete from product where p_name = '무';
--가장 좁은 매장 상품 다 삭제
delete from product where p_m_no = (select m_no from mart where m_scale = (select max(m_scale)from mart));
